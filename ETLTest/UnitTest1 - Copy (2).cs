      ...
      bufferSize: 4096, useAsync: true);
       ...
       bufferSize: 4096, useAsync: true);
        ...
        private async Task<CallbackModelWrapper> PopulateImagesAsync(CallbackModelWrapper model, int amount)
        {
            ...
            var tasks_image = Enumerable.Range(0, amount).Select(index => ReadOneImage(model2, 28, offset_origin_image + index * 28 * 28, offset_origin_label + index, index + 1));
        ...
        private async Task<DigitImage> ReadOneImage(CallbackModelWrapper model, int imageSize, int offset_image, int offset_label, int orderInCollection)
        {
